...
- **No more `'unsafe-eval'` — anywhere.** Through 2.0.x, validated `:computed` / `@loop … where` / `.class when` / `:if` expressions compiled to JS fragments evaluated via `new Function`, which forced `script-src 'unsafe-eval'` onto reactive pages. The compiler now serializes each already-validated expression into a compact AST emitted in the `data-wd-*` attributes, and the runtime walks it with a tiny closed evaluator — no `new Function`, no `eval`. Compile-time validation (the security layer) is unchanged; the evaluator applies the actual JS operators, so semantics are identical to the old path — guarded by a parity battery comparing old-vs-new results across a wide input matrix (`tests/expr-ast.test.js`), plus the fuzz suite proving hostile text survives only as inert literal data. `REACTIVE_CSP` is now byte-identical to `STATIC_CSP` across `dist/_headers`, `vercel.json`, and local `serve`. Cost: the runtime grows 7518 → **7654 B gzipped**, still well under the 8 KB budget. Compiled-page attribute payloads changed shape, but the runtime always ships with its build, so nothing to migrate.
- **Browser playground at [darkmown.com/playground/](https://darkmown.com/playground/).** Editor + live preview: the actual compiler (markdown-it and a common-languages highlight.js subset bundled to one ~128 KB gzipped asset that loads only on that page) compiles your `.wd`/`.md` in the browser on every keystroke, renders into an iframe, and shows the real `file:line` compile errors — the error DX is part of the demo. Reactive previews run the real `/__wd/runtime.js` under the shipped strict CSP. Linked from the hero ("or try it in your browser") and the docs.
- **`compileFromMemory(files, entryPath, options)` — compile with no filesystem.** A public API taking a map of project-relative paths → source strings; includes, colocated `.skin`/`.js` detection, and `@loop` JSON data all resolve against the map, with identical `file:line` errors. Under the hood the compiler's ~11 `node:fs` call sites now go through an injectable reader (`src/compiler/fs-reader.js` is the only compiler module importing `node:fs`), so the whole compile path runs in any JS engine — the playground uses it today; an on-device mobile app can use it tomorrow. The on-disk build path is byte-identical (golden-verified against a full demo-site build).
- **Internal:** `esbuild` added as a devDependency solely to build the playground bundle (`npm run build:playground`); the bundle is committed so `npm run build` needs no bundler step.
...
- **Build-output containment guard.** `outputPathForRoute` now throws if a route would resolve its `index.html` outside the build output directory — defense in depth behind the route-discovery walker, which cannot produce traversal segments today.
- **`lang:` frontmatter.** A page sets its document language with `lang: fr` (any BCP-47 tag) and the shell emits `<html lang="fr">` (escaped); the default stays `en`.
- **Skip-to-content link + guaranteed main landmark.** Every page's first focusable element is a visually-hidden-until-focused "Skip to content" link. The shell reuses your `<main>` (an author id wins as the skip target; an id-less `<main>` gets `id="main"` stamped on — a `data-id` or similar `-id`-suffixed attribute is never mistaken for the real id) or wraps a main-less body in `<main id="main">`. Compile-time only — static pages stay `runtime: false`. Restyle via `.wd-skip-link`; the site base skin ships a themed focus reveal.
- **`:fetch` lifecycle regions are announced.** A bare `:if name_loading` over a fetch key compiles to a `role="status" aria-live="polite"` region and `:if name_error` to `role="alert"`, so the runtime's existing show/hide flips are read by assistive tech with zero added runtime bytes. Author-supplied `role`/`aria-live` inside a region is never overridden.
- **Heading anchors + a docs table of contents.** Every markdown heading now carries a stable, GitHub-style slug `id` at build time (lowercased, punctuation stripped, whitespace → hyphens; duplicates dedupe with `-1`/`-2` suffixes across the whole document, even when directives split the prose into chunks or a `.md` include repeats a heading from the including page), so any section of any page is deep-linkable with a plain `#` fragment and zero JS. The docs page opens with an "On this page" TOC built from those anchors — static markdown links, so `/docs/` stays `runtime: false` — and a dogfood test verifies every TOC fragment against the built page's real ids. The docs Install section also points at the VS Code extension's install-from-VSIX flow.
...
- **Compile errors report true file line numbers.** Line numbers were computed over the frontmatter-stripped body, so an error after a 3-line frontmatter block pointed three lines short. The frontmatter offset is now threaded through the compile context, so `file:line` matches what an editor's jump-to-line expects. The same offset also threads through every **nested block body** — a directive inside a `::: container`, `@loop` rows or `@empty` branch, `:if`/`:else if`/`:else` branch, `:form`, or `:carousel` previously reported a line relative to the sliced block body (wrong but plausible); it now reports the true file line at any nesting depth.
- **Incremental dev rebuilds.** `darkmown dev` now tracks a per-route dependency map (source file, includes, colocated `.skin`/`.js` assets, `@loop` JSON data, collections looped — written to `dist/.wd-dev-deps.json` on every dev build) and rebuilds only the routes affected by a `site/` content change: editing an include recompiles just the pages that include it, editing a collection entry recompiles the entry plus the collection's listing/paginated pages, and `routes.json`/`_headers`/sitemap/rss are re-emitted globally every time so partial rebuilds stay whole-site consistent. Any uncertainty — a new/deleted/renamed file, a file the map doesn't account for, a change to the site-wide feed link — falls back to a full rebuild, and a framework `src/` change still always runs the full child-process rebuild. Rebuild child processes are **serialized** — at most one runs at a time, with changes arriving mid-build coalescing into the next batch — because every build read-modify-writes the dependency map, and overlapping children could persist a stale map that silently stops affected routes from rebuilding. The dev log says what happened: `Rebuilt 2 of 29 routes (/blog/, /blog/hello/) into dist`. Production `darkmown build` is unchanged and writes no map.
...
- **Blog template runs on content collections.** `darkmown init --template blog` no longer ships a hand-maintained `site/_/posts.json` manifest (whose dates contradicted the posts' frontmatter): `site/pages/posts/` is the collection, the home page is one `@loop posts into post sort by post.date desc` with an `@empty` branch, and a `_schema.wd` type-checks every post's frontmatter at build time. Adding a post is now just adding a `.md` file. Also fixes the listing's date/excerpt, which the old raw-HTML card markup rendered as literal `{ post.date }` text.
- **Scaffolded `.md` pages are styled.** The starter's `about.md` and the blog template's posts now ship colocated `.skin` siblings (the same colocation that styles a `.wd` page — no new mechanism) plus visible navigation back home, so a fresh template has no unstyled dead-end pages. The pages stay `runtime: false` with zero scripts, guarded by a template test.
- **The repo's own demo blog posts are styled.** `site/pages/blog/*.md` (the posts on darkmown.com) were rendering as bare, unstyled HTML because a plain `.md` post can't `@include /base.wd`. Each post now carries a colocated `.skin` reading layer that mirrors the site's base tokens (serif prose, warm-literary palette, OS-driven dark mode) — closing the same gap the scaffold templates just fixed. The posts stay `runtime: false` with zero scripts, guarded by a new dogfood test asserting every built blog post links a stylesheet.
- **Fixed: the `:fetch` e2e harness now boots the runtime under the shipped CSP.** With the reactive CSP dropping `'unsafe-inline'` from `script-src`, the fetch spec's self-contained harness — which injected the runtime as an inline `<script>` — was blocked. It now injects the runtime by URL (`/__wd/runtime.js`, same-origin, covered by `script-src 'self'`), matching how the other e2e specs load it and exercising the real shipped runtime. Product code and the CSP are unchanged; only the test harness was.
...
- **Fixed: date pipes are now deterministic across build machines.** A date-only value like `{ "2026-06-22" | date:"medium" }` parses as UTC midnight, so formatting it in the build machine's local zone could render `Jun 21, 2026` west of UTC. Date-only strings (`YYYY-MM-DD`) now format with `timeZone: 'UTC'` in both the compiler fold and the runtime mirror, so the output matches the written calendar date everywhere; full datetimes keep their local-zone behavior. This is the one runtime change in 2.0.0 (7459 → 7518 B gzipped, still well under the 8 KB budget); `tests/format-parity.test.js` keeps the compiler and runtime formatters in lockstep.
- **Fixed: chained `:fetch` lifecycle regions keep their live-region roles.** In the documented four-state pattern (`:if x_loading` / `:else if x_error` / `:else`), the loading region's compiled falsy branch contains the nested error region's framework-added `role="alert"` — which the author-aria guard mistook for author-supplied aria and used to suppress the loading region's own `role="status" aria-live="polite"`. The guard now inspects only the region's own source, so a genuine author `role`/`aria-live` still wins but a nested framework attribute no longer silences the outer region.
- **`darkmown serve` now applies the per-route CSP, matching production.** The local preview server previously served the relaxed (reactive) CSP to *every* route, so a static page got `script-src 'unsafe-eval'` locally that its production `_headers`/`vercel.json` would not grant. It now reads the build's `routes.json` and serves each route the same strict (static) or relaxed (reactive) policy production ships; an unknown route or missing manifest falls back to the reactive policy, exactly like the production catch-all. `darkmown dev` is unchanged.
- **Clearer feed build hints.** `darkmown build` now warns when `site_url` is still the `https://example.com` placeholder (the blog template's default) so feeds/sitemap don't silently point at example.com, and the existing "set `site_url`" hint prints once per `darkmown dev` session instead of on every incremental rebuild.
- **Internal: the directive-handler module was split along its cohesive seams** (`state`/`actions`/`fetch`/`forms`/`media`/`structure` under `src/compiler/`, with `directives.js` now a re-export barrel), and the compiler's import cycles were removed — block handlers and the loop pipeline recurse through `ctx.compileBody`/`ctx.compileFile` instead of importing the dispatcher back, and `stampScope` moved beside the scoped-skin detection in `includes.js`. Pure refactor: no public API, output, or behavior change.
