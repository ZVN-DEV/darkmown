...
- **Skip-to-content link + guaranteed main landmark.** Every page's first focusable element is a visually-hidden-until-focused "Skip to content" link. The shell reuses your `<main>` (an author id wins as the skip target; an id-less `<main>` gets `id="main"` stamped on) or wraps a main-less body in `<main id="main">`. Compile-time only — static pages stay `runtime: false`. Restyle via `.wd-skip-link`; the site base skin ships a themed focus reveal.
