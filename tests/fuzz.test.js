...
function randLine(rng) {
  ...
  switch (kind) {
    ...
    case 8: return `:else`;
    case 9: return `:endif`;
    case 10: return `:computed ${id} = ${randExpr(rng)}`;
    case 11: return `:button ${rng.pick(['"Go" -> ' + id + ' += 1', '"X"', "", '"Y" -> badAction()'])}`;
    case 12: return `@include ${rng.pick(["/nav.wd", "../../etc/passwd", "/missing.wd", "/loop.wd", ""])}`;
    case 13: return `::: ${rng.pick(["card", "note", "", "  "])}`;
    case 14: return `:::`;
    case 15: return "```" + (rng.bool() ? "js" : "");
    case 16: return rng.pick(["# Heading", "- bullet", "> quote", "| a | b |", "plain prose", "**bold**"]);
    case 17: return `:fetch ${id} from ${rng.pick(['"/data.json"', "/data.json", ""])}`;
    case 18: return `:input ${id} ${rng.pick(["", "type=email", "required", "bogusflag", 'placeholder="x"'])}`;
    case 19: return `:bind ${rng.pick([id, ""])}`;
    case 20: return `:form ${rng.pick(["into " + id, 'action="/x"', "", "garbage"])}`;
    case 21: return `:submit ${rng.pick(['"Send"', ""])}`;
    case 22: return rng.pick(["@repeat x", ":for x", ":endfor"]);  // removed/deprecated → should throw
    case 23: return rng.pick(UNICODE_BITS).repeat(1 + num);
    case 24: return `{ ${rng.pick(["meta.title", "x.constructor", "a.__proto__.b", "deep.nested.path"])} }`;
    case 25: return `:computed ${id} = ${randDangerExpr(rng)}`;
    case 26: return "---";                                       // stray frontmatter fence inside body
    case 27: return rng.pick(["", "   ", "\t\t"]);
    default: return "";
  }
}
...
function randDoc(rng) {
  ...
  if (rng.bool(0.3)) {
    if (rng.bool(0.7)) {
      lines.push("---", `title: ${rng.pick(["Hi", '"Quoted"', "[a, b]", ""])}`, `html: ${rng.pick(["false", "true", "x"])}`, "---");
    } else {
...
const SAFE_TOKENS = ["count", "items", "x", "1", "42", "3.14", "+", "-", "*", "/", "%", "(", ")", "<", ">", "==", "!=", "&&", "||", "true", "false", "null"];
const DANGER_TOKENS = ["`", "process", "constructor", "__proto__", "prototype", "=", "alert(1)", "this", "global", "require", "import", "x.constructor", "a=b", "${x}", "() => 1", "function", "[].map"];
...
function randDangerExpr(rng) {
  ...
  let hasDanger = false;
  for (let i = 0; i < n; i++) {
    if (rng.bool(0.5)) { parts.push(rng.pick(DANGER_TOKENS)); hasDanger = true; }
...
const DANGEROUS_ARTIFACT = /\b(process|constructor|prototype|__proto__|require|global|globalThis)\b|`|=>|\$\{/;
...
test("fuzz: compilePage/compileDocument never crash and never emit undefined", () => withExpectedCompilerWarnings(() => {
  ...
  for (let i = 0; i < CASES; i++) {
    ...
    try {
      const result = useShell ? compilePage(pageFile, context) : compileDocument(pageFile, context);
  ...
  for (let i = 0; i < CASES; i++) {
    ...
    try {
      ...
      assert.ok(
        ...
        repro(`a dangerous token survived into the compiled artifact that feeds new Function:\n  artifact: ${artifact}`)
      );
