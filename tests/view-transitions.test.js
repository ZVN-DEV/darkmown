...
test("transitions: true emits the @view-transition opt-in in <head>", () => {
  ...
  write(
    root,
    "site/pages/index.wd",
    ["---", "transitions: true", "---", "<main>", "", "Hi", "", "</main>"].join("\n")
  );
  ...
  assert.match(
    ...
    "the @view-transition opt-in must live in <head>"
  );
...
test("transitions: true overrides the default root cross-fade with a directional fade+slide", () => {
  ...
  assert.match(
    html,
    /::view-transition-old\(root\)[^}]*animation:[^};]*wd-nav-out/,
    "old(root) must use the custom leave animation"
  );
  assert.match(
    html,
    /::view-transition-new\(root\)[^}]*animation:[^};]*wd-nav-in/,
    "new(root) must use the custom enter animation"
  );
  ...
  assert.match(
    html,
    /@keyframes wd-nav-out\s*\{[^}]*translateY/,
    "wd-nav-out keyframes must slide"
  );
...
test("transitions: false explicitly opts out", () => {
  ...
  write(
    root,
    "site/pages/index.wd",
    ["---", "transitions: false", "---", "<main>", "", "Hi", "", "</main>"].join("\n")
  );
  const page = compilePage(path.join(root, "site/pages/index.wd"), createPaths(root));
